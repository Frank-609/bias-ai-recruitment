...
from src.common import DATA_DIR, OUTPUTS_DIR, bootstrap, read_jsonl
...
def filter_valid_results(results: list[dict]) -> list[dict]:
    valid_session_ids = {row['session_id'] for row in read_jsonl(SESSIONS_PATH)}
    valid_cv_ids = {row['cv_id'] for row in read_jsonl(VARIANTS_PATH)}

    cleaned: list[dict] = []
    dropped_invalid_session = 0
    dropped_invalid_cvs = 0

    for row in results:
        session_id = row.get('session_id')
        if session_id not in valid_session_ids:
            dropped_invalid_session += 1
            continue

        cv_ids = row.get('cv_ids', [])
        if not isinstance(cv_ids, list) or len(cv_ids) != 2:
            dropped_invalid_cvs += 1
            continue

        if any(cv_id not in valid_cv_ids for cv_id in cv_ids):
            dropped_invalid_cvs += 1
            continue

        cleaned.append(row)

    if dropped_invalid_session or dropped_invalid_cvs:
        logging.warning(
            'Filtered out %d results with invalid session_id and %d results with invalid cv_ids',
...
def extract_pair_metrics(row: dict) -> dict | None:
    ...
    return {
        ...
        'scenario': row.get('scenario', 'single_attribute'),
        'bias_variable': row.get('bias_variable'),
        'variant_1': v1,
        'variant_2': v2,
        'winner': winner,
        'score_delta_v1_minus_v2': score_delta,
    }
...
def aggregate_metrics(results: list[dict]) -> list[dict]:
    ...
    for (provider, model, prompt_id, scenario, bias_variable), rows in grouped.items():
        ...
        summaries.append({
            ...
            'dominant_variant_binomial_pvalue': (
                binomial_two_sided_pvalue(win_counts.get(dominant_variant, 0), total_decisions)
                if dominant_variant
                else None
            ),
        })
...
def plot_bar(
    ...
    plt.title(
        f"{title_prefix}\n"
        f"{metric['provider']} | {metric['scenario']} | {metric['bias_variable']} | {metric['prompt_id']}"
    )
    ...
    out_path = FIGURES_DIR / (
        f"{filename_prefix}_{metric['provider']}_{metric['scenario']}_{metric['bias_variable']}_{metric['prompt_id']}.png"
    )
...
def print_summary(metrics: list[dict]) -> None:
    for metric in metrics:
        logging.info(
            'provider=%s | model=%s | scenario=%s | prompt=%s | bias=%s | n=%d | dominant=%s | p=%s | mean_abs_delta=%s',
            metric['provider'],
            metric['model'],
            metric['scenario'],
            metric['prompt_id'],
            metric['bias_variable'],
            metric['n_sessions'],
            metric['dominant_variant'],
            metric['dominant_variant_binomial_pvalue'],
...
def main() -> None:
    ...
    if not results:
        raise ValueError('No valid results remain after filtering against current sessions and variants')
